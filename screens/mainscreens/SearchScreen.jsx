...
export default function SearchScreen() {
    const navigation = useNavigation();
    const [activeTab, setActiveTab] = useState(null); // null = discover
    const [query, setQuery] = useState("");
    ...
    // Live search (tab + query)
    const { data: searchData, isLoading: searchLoading, isError: searchError } =
        useSearch(activeTab, query, { enabled: !!activeTab && !!query });

    const searchResults = searchData?.data?.data || [];
    const searchTotal = searchData?.data?.total ?? searchResults.length;

    // Local filtering for discover sections (unchanged)
    const filteredProducts = useMemo(() => {
        if (!query.trim()) return productData?.data || [];
        const q = query.toLowerCase();
        return (productData?.data || []).filter(p => p.name?.toLowerCase().includes(q));
    }, [productData, query]);

    const filteredStores = useMemo(() => {
        if (!query.trim()) return storeData?.data || [];
        const q = query.toLowerCase();
        return (storeData?.data || []).filter(s => s.store_name?.toLowerCase().includes(q));
    }, [storeData, query]);

    const filteredServices = useMemo(() => {
        if (!serviceData?.data) return [];
        const q = query.toLowerCase();
        return (serviceData.data || []).filter(s => s.name?.toLowerCase().includes(q));
    }, [serviceData, query]);
          ...
          />
            ...
            {/* <View style={styles.section}>
                ...
                <FlatList
                    ...
                    keyExtractor={(item) => item.id.toString()}
                    renderItem={({ item }) => {
                        const cover = item.banner_image
                            ? { uri: imgUrl(item.banner_image) }
                            : require("../../assets/Rectangle 30.png");
                        const avatar = item.profile_image
                            ? { uri: imgUrl(item.profile_image) }
                            : require("../../assets/Ellipse 18.png");
                        return (
                            <View style={styles.topStoreCard}>
                                <Image source={cover} style={styles.topStoreCover} />
    ...
    const renderContent = () => {
        ...
        if (searchLoading) return <ActivityIndicator style={{ marginTop: 40 }} />;
        if (searchError)
            return (
                <View style={{ padding: 16 }}>
                    <Text style={{ color: "#c00" }}>
                        Search failed. Check type mapping or query.
                    </Text>
                </View>
            );
        ...
        return (
            <>
                {query ? (
                    <Text style={styles.resultCount}>Search Results ({searchTotal})</Text>
                ) : null}
                <FlatList
                    data={searchResults}
                    keyExtractor={(item) => item.id.toString()}
...
/* ---------- Styles (unchanged from your last) ---------- */
const styles = StyleSheet.create({
    header: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingHorizontal: 16, paddingVertical: 16, backgroundColor: "#fff" },
    headerTitle: { fontSize: 18, fontWeight: "700", color: COLOR.text },
    backBtn: { padding: 6, borderWidth: 0.3, borderColor: "#ccc", borderRadius: 20 },
    ...
    tabText: { color: COLOR.text, fontSize: 10 },
    tabTextActive: { color: "#fff", fontWeight: "600" },
    resultCount: { marginLeft: 16, marginBottom: 8, fontSize: 14, fontWeight: "600", color: COLOR.text },
    section: { paddingHorizontal: 16, marginBottom: 20 },
    sectionTitle: { fontSize: 16, fontWeight: "600", marginBottom: 12, color: COLOR.text },
    ...
    tagRow: { flexDirection: "row", gap: 8, marginTop: 8, marginBottom: 10 },
    tag: { paddingHorizontal: 6, paddingVertical: 3, borderRadius: 5, borderWidth: 1 },
    blueTag: { borderColor: "#3D71FF", backgroundColor: "#E9F0FF" },
    redTag: { borderColor: COLOR.primary, backgroundColor: "#FFE7E6" },
    tagText: { fontSize: 8, fontWeight: "600" },
    goShopBtn: { backgroundColor: COLOR.primary, height: 34, borderRadius: 10, alignItems: "center", justifyContent: "center" },
    goShopText: { color: "#fff", fontSize: 10, fontWeight: "400" },
