...
export default function SearchScreen() {
    ...
    const renderServiceCard = ({ item }) => {
        ...
        return (
            <View style={styles.serviceCard}>
                <Image source={cover} style={styles.serviceImage} />
                <View style={styles.serviceHeader}>
                    {/* No store avatar in API → keep hardcoded */}
                    <Image
                        source={require("../../assets/Ellipse 18.png")}
                        style={styles.serviceAvatar}
                    />
                    <ThemedText style={styles.storeName}>{item.name}</ThemedText>
                    {/* No rating in API → hardcode */}
                    <View style={styles.ratingRow}>
                        <Ionicons name="star" size={14} color={COLOR.primary} />
                        <ThemedText style={styles.ratingText}>4.5</ThemedText>
                    </View>
    ...
    const renderContent = () => {
        ...
        if (activeTab === "services") {
            if (servicesLoading) return <ActivityIndicator style={{ marginTop: 40 }} />;
            return (
                <FlatList
                    data={filteredServices}
                    keyExtractor={(item) => item.id.toString()}
