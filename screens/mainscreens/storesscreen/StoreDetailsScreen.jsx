...
// app/screens/store/StoreDetailsScreen.jsx
...
//   useAddStoreReview,
//   fileUrl,
// } from "../../../config/api.config"; // ⬅️ adjust the import path if your api.config.js lives elsewhere
...
// /* Helpers: allow Image to accept require(...) or URL */
// const toSrc = (v) => (typeof v === "number" ? v : v ? { uri: String(v) } : undefined);
// const na = (v, fallback) => (v == null || v === "" ? fallback : v);
// const money = (n) => {
//   const num = Number(n);
...
//     } catch {}
//   }
//   const storeIdFromParams = route?.params?.id || store?.id || 1; // default to 1 so your shared link works

//   // Fetch store details from API
//   const { data: storeRes } = useStoreDetails(storeIdFromParams);
//   const apiStore = storeRes?.data?.data || null;

//   // Adapt backend payload to your component's expectations (without removing any original fallbacks)
//   const uiStore = apiStore
//     ? {
//         id: apiStore.id,
//         name: na(apiStore.store_name, store?.name || "Store"),
//         email: na(apiStore.store_email, "sashastores@gmail.com"),
//         phone: na(apiStore.store_phone, "070123456789"),
//         location: na(apiStore.store_location, "Lagos, Nigeria"),
//         avatar: fileUrl(apiStore.profile_image) || store?.avatar,
//         cover: fileUrl(apiStore.banner_image) || store?.cover,
//         rating: apiStore.rating ?? store?.rating ?? 0,
//         total_sold: apiStore.total_sold ?? null,
//         followers_count: Array.isArray(apiStore.followers) ? apiStore.followers.length : null,
//         social_links: apiStore.social_links || [],
//         products: (apiStore.products || []).map((p) => {
//           const mainImg =
//             p.images?.find((im) => String(im.is_main) === "1") || (p.images?.length ? p.images[0] : null);
//           return {
//             id: p.id,
//             title: p.name,
//             name: p.name,
//             price: money(p.price) || money(p.variations?.[0]?.price) || null, // prefer main price; fallback to variant
//             originalPrice: null, // not provided
//             rating: apiStore.rating ?? 0, // no per-product rating in payload
//             image: fileUrl(mainImg?.path),
//             store: na(apiStore.store_name, store?.name || "Store"),
//             store_image: fileUrl(apiStore.profile_image) || store?.avatar,
//             location: na(apiStore.store_location, "Lagos, Nigeria"),
//             sponsored: false, // not provided
//             tagImages: [], // not provided
//           };
//         }),
//         posts: apiStore.posts || [],
//         store_reviews: apiStore.store_reveiews || [], // (backend key as sent)
//       }
//     : null;

//   // prefer API store when available
//   const activeStore = uiStore || store;

//   const coverSrc = toSrc(activeStore?.cover);
//   const avatarSrc = toSrc(activeStore?.avatar);
...
//       id: "1",
//       title: "Dell Inspiron Laptop",
//       store: activeStore?.name || "Sasha Stores",
//       store_image: activeStore?.avatar || require("../../../assets/Ellipse 18.png"),
//       location: "Lagos, Nigeria",
//       rating: 4.5,
//       price: "₦2,000,000",
//       originalPrice: "₦3,000,000",
...
//   const promoSrc = toSrc(activeStore?.promo) || require("../../../assets/storeimage.png");

//   const productsSource =
//     Array.isArray(activeStore?.products) && activeStore.products.length ? activeStore.products : DEMO_PRODUCTS;
...
//       store: activeStore?.name || "Sasha Stores",
//       avatar:
//         typeof activeStore?.avatar === "number"
//           ? activeStore.avatar
//           : activeStore?.avatar || "https://images.unsplash.com/photo-1544005313-94ddf0286df2?q=80&w=200&auto=format&fit=crop",
//       location: "Lagos, Nigeria",
//       timeAgo: "20 min ago",
//       images: [
//         "https://images.unsplash.com/photo-1511707171634-5f897ff02aa9?q=80&w=1200&auto=format&fit=crop",
...
//         typeof activeStore?.avatar === "number"
//           ? activeStore.avatar
//           : activeStore?.avatar || "https://images.unsplash.com/photo-1547425260-76bcadfb4f2c?q=80&w=200&auto=format&fit=crop",
//       location: "Lagos, Nigeria",
//       timeAgo: "20 min ago",
...
//     },
//   ];
//   const postsSource = Array.isArray(activeStore?.posts) && activeStore.posts.length ? activeStore.posts : DEMO_POSTS;
...
//     const q = query.toLowerCase();
//     return productsSource.filter(
//       (p) => (p?.title || p?.name || "").toLowerCase().includes(q) || (p?.store || "").toLowerCase().includes(q)
//     );
//   }, [query, productsSource]);
...
//           {
//             id: "rs2-r1",
//             user: activeStore?.name || "Sasha Stores",
//             avatar:
//               typeof activeStore?.avatar === "number"
//                 ? activeStore.avatar
//                 : activeStore?.avatar ||
...
//   const { data: revRes } = useStoreReviews(storeIdFromParams);
//   const serverReviews = revRes?.data || [];

//   const mapServerReview = (rv) => ({
//     id: String(rv.id),
//     user: rv.user?.full_name || "User",
//     avatar:
//       fileUrl(rv.user?.profile_picture) ||
//       "https://images.unsplash.com/photo-1502767089025-6572583495b0?q=80&w=200&auto=format&fit=crop",
//     rating: Number(rv.rating) || 0,
//     time: rv.created_at?.slice(0, 16)?.replace("T", "/") || "",
//     text: rv.comment || "",
//     replies: [],
//   });
...
//     serverReviews.length ? serverReviews.map(mapServerReview) : DEMO_REVIEWS.store
//   );
//   const [reviewsProduct, setReviewsProduct] = useState(DEMO_REVIEWS.product);

//   useEffect(() => {
//     if (serverReviews?.length) {
//       setReviewsStore(serverReviews.map(mapServerReview));
//     }
//     // if empty, keep your demo reviews
//   }, [revRes]);
...
//     onSuccess: (res) => {
//       const rv = res?.data?.data;
//       if (rv) {
//         setReviewsStore((prev) => [mapServerReview(rv), ...prev]);
//       }
//     },
//   });
...
//         rating: rating ?? 0,
//         comment: text || "",
//         images: [], // pass image URIs if you wire an image picker
//       });
//     } catch (e) {
//       // graceful fallback: keep your local demo behavior
//       const newRev = {
//         id: `rs-${Date.now()}`,
//         user: activeStore?.name || "Sasha Stores",
//         avatar:
...
//   const activeReviews = reviewScope === "store" ? reviewsStore : reviewsProduct;
//   const reviewCount = activeReviews.length;
//   const avgRating = reviewCount ? activeReviews.reduce((a, r) => a + (r.rating || 0), 0) / reviewCount : 0;
...
//             <View style={[styles.storeAvatar, { backgroundColor: "#ddd" }]} />
//           )}
//           <ThemedText style={styles.storeName}>{item?.store || activeStore?.name || "Store"}</ThemedText>
//         </View>
//         <View style={styles.ratingRow}>
//           <Ionicons name="star" color="#E53E3E" size={12} />
//           <ThemedText style={styles.ratingTxt}>{item?.rating ?? activeStore?.rating ?? "4.5"}</ThemedText>
//         </View>
//       </View>
...
//             >
//               {images.map((uri, idx) => (
//                 <Image key={`${item.id}-img-${idx}`} source={toSrc(uri)} style={[styles.postImage, { width: carouselW }]} />
//               ))}
//             </ScrollView>
//           )}
...
//     const [replyTo, setReplyTo] = useState(null); // { commentId, username }
//     const currentUser = {
//       name: activeStore?.name || "Sasha Stores",
//       avatar:
//         typeof activeStore?.avatar === "number"
//           ? activeStore.avatar
//           : activeStore?.avatar || "https://images.unsplash.com/photo-1544005313-94ddf0286df2?q=80&w=200&auto=format&fit=crop",
...
//         };
//         setComments((prev) =>
//           prev.map((c) => (c.id === replyTo.commentId ? { ...c, replies: [...(c.replies || []), newReply] } : c))
//         );
//         setReplyTo(null);
//         setText("");
//       } else {
...
//             <Row icon={<Ionicons name="warning-outline" size={20} color={COLOR.primary} />} label="Report Post" danger onPress={onClose} />
//           </View>
//         </View>
//       </Modal>
//     );
...
//         {/* Cover */}
//         <View style={styles.coverWrap}>
//           {coverSrc ? <Image source={coverSrc} style={styles.cover} resizeMode="cover" /> : <View style={[styles.cover, { backgroundColor: "#e9e9e9" }]} />}
...
//           {avatarSrc ? <Image source={avatarSrc} style={styles.avatar} /> : <View style={[styles.avatar, { backgroundColor: "#fff" }]} />}
//         </View>

//         <View className="status-row" style={styles.statusRow}>
//           <View style={styles.statusPill}>
//             <Ionicons name="ellipse" size={8} color="#fff" />
//             <ThemedText style={styles.statusTxt}>Open Now · 07:00AM - 08:00PM</ThemedText>
//             <TouchableOpacity style={styles.followBtn}>
...
//           <View style={styles.rowBetween}>
//             <View style={{ flexDirection: "row", alignItems: "center", gap: 8, flex: 1 }}>
//               <ThemedText style={[styles.storeName, { fontSize: 16, fontWeight: 500, color: "#000" }]}>{activeStore?.name || "Store"}</ThemedText>
//               {/* <Ionicons name="shield-checkmark" size={16} color={COLOR.primary} /> */}
//             </View>
//           </View>
...
//           <View style={styles.metaRow}>
//             <Ionicons name="mail-outline" size={16} color={COLOR.sub} />
//             <ThemedText style={styles.metaTxt}>{activeStore?.email || "sashastores@gmail.com"}</ThemedText>
//           </View>
//           <View style={styles.metaRow}>
//             <Ionicons name="call-outline" size={16} color={COLOR.sub} />
//             <ThemedText style={styles.metaTxt}>{activeStore?.phone || "070123456789"}</ThemedText>
//           </View>
//           <View style={styles.metaRow}>
//             <Ionicons name="location-outline" size={16} color={COLOR.sub} />
//             <ThemedText style={[styles.metaTxt, { marginBottom: 5 }]}>{activeStore?.location || "Lagos, Nigeria"}</ThemedText>
//             <ThemedText style={[styles.metaTxt, { color: COLOR.primary, textDecorationLine: "underline" }]}>View Store Addresses</ThemedText>
//           </View>

//           {!!activeStore?.tags && Array.isArray(activeStore.tags) && activeStore.tags.length > 0 && (
//             <View style={styles.tagsRow}>
//               <Ionicons name="call-outline" size={16} color={COLOR.sub} />
//               <ThemedText style={styles.metaTxt}>Category</ThemedText>
//               {activeStore.tags.map((t, i) => (
//                 <View key={`${t}-${i}`} style={[styles.tag, i === 0 ? styles.tagBlue : styles.tagRed]}>
//                   <ThemedText style={[styles.tagTxt, i === 0 ? styles.tagTxtBlue : styles.tagTxtRed]}>{t}</ThemedText>
//                 </View>
//               ))}
...
//               <View>
//                 <ThemedText style={styles.statLabel}>Qty Sold</ThemedText>
//                 <ThemedText style={styles.statValue}>{activeStore?.total_sold ?? 100}</ThemedText>
//               </View>
//             </View>
...
//               <View>
//                 <ThemedText style={styles.statLabel}>Followers</ThemedText>
//                 <ThemedText style={styles.statValue}>{activeStore?.followers_count ?? 500}</ThemedText>
//               </View>
//             </View>
...
//               <View>
//                 <ThemedText style={styles.statLabel}>Ratings</ThemedText>
//                 <ThemedText style={styles.statValue}>{activeStore?.rating ?? 4.7}</ThemedText>
//               </View>
//             </View>
//           </View>
...
//         {/* Social icons – images (opens real links if present) */}
//         <View style={styles.socialCard}>
//           {[
//             { id: "wa", uri: "https://img.icons8.com/color/48/whatsapp--v1.png", link: activeStore?.social_links?.find((s) => s.type === "whatsapp")?.url },
//             { id: "ig", uri: "https://img.icons8.com/color/48/instagram-new--v1.png", link: activeStore?.social_links?.find((s) => s.type === "instagram")?.url },
//             { id: "x", uri: "https://img.icons8.com/ios-filled/50/x.png", link: null },
//             { id: "fb", uri: "https://img.icons8.com/color/48/facebook-new.png", link: null },
//           ].map((s) => (
//             <TouchableOpacity key={s.id} style={styles.socialBtn} onPress={() => s.link && Linking.openURL(s.link)}>
//               <Image source={{ uri: s.uri }} style={styles.socialImg} />
//             </TouchableOpacity>
//           ))}
//         </View>
...
//                     typeof activeStore?.avatar === "number"
//                       ? activeStore.avatar
//                       : activeStore?.avatar || require("../../../assets/Ellipse 18.png"),
//                 },
//               })
//             }
//           >
...
//             style={styles.bigBtnTxt}>Chat</ThemedText>
//           </TouchableOpacity>

//           <TouchableOpacity style={[styles.bigBtn, styles.bigBtnGreen]} onPress={() => setLeaveReviewVisible(true)}>
//             <ThemedText style={styles.bigBtnTxt}>Leave a store review</ThemedText>
//           </TouchableOpacity>
//         </View>
...
>>>>>>> e31d9f3c31bb62e361738ac59ca9ec220222a8c4
import React, { useMemo, useState, useRef, useEffect } from "react";
import {
  View,
  StyleSheet,
