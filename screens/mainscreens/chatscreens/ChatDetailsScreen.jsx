...
// screens/.../ChatDetailsScreen.jsx
import React, { useEffect, useMemo, useRef, useState } from 'react';
import {
  View, Image, FlatList, TextInput,
  TouchableOpacity, KeyboardAvoidingView, Platform, Keyboard, Modal, Pressable, StyleSheet
} from 'react-native';
import { SafeAreaView, useSafeAreaInsets } from 'react-native-safe-area-context';
import { Ionicons } from '@expo/vector-icons';
import { useNavigation, useRoute } from '@react-navigation/native';
import ThemedText from '../../../components/ThemedText';
import { useChatMessages, useSendChatMessage, queryClient } from '../../../config/api.config';
...
export default function ChatDetailsScreen() {
  ...
  // passed from chat list
  const avatarSrc = toSrc(store?.profileImage) || toSrc('https://i.pravatar.cc/100?img=65');
  const insets = useSafeAreaInsets();
  ...
  const { data: apiRes } = useChatMessages(chatId);
  const apiMsgs = apiRes?.data || [];
  const mappedApi = useMemo(() => {
    const sorted = [...apiMsgs].sort((a, b) => new Date(a.created_at) - new Date(b.created_at));
    return sorted.map((m) => ({
      id: m.id,
      text: m.message ?? '(image)', // API can send image-only; keep placeholder text per instruction
  ...
  const handleSend = () => {
    ...
    setMessages((prev) => [...prev, { id: tempId, text: v, sender: 'me', time }]);
    setInputText('');
    scrollToEnd();
  ...
  const [issueDetails, setIssueDetails] = useState('');
  const [imageUri, setImageUri] = useState(null);
  const pickImage = () => {}; // no-op (images not required now)

  const resetDisputeForm = () => { setIssueCategory(''); setIssueDetails(''); setImageUri(null); };
  const submitDispute = () => {
    if (!issueCategory || !issueDetails.trim()) return;
    const time = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: true }).toUpperCase();
    setMessages(prev => [
      ...prev,
      { id: Date.now(), type: 'dispute', sender: 'me', time, payload: { category: issueCategory, details: issueDetails.trim(), imageUri } }
    ]);
    setShowDispute(false);
    resetDisputeForm();
    scrollToEnd();
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'} keyboardVerticalOffset={KAV_OFFSET}>
        ...
        <View style={styles.header} onLayout={e => setHeaderH(e.nativeEvent.layout.height)}>
          ...
          <View style={{ flexDirection: 'row', gap: 8 }}>
            ...
            <TouchableOpacity style={styles.hIcon}>
              <Image source={require('../../../assets/msg-cart.png')} style={styles.iconImg} />
            </TouchableOpacity>
          </View>
        </View>
        ...
        <Modal visible={menuOpen} transparent animationType="fade" onRequestClose={() => setMenuOpen(false)}>
          <Pressable style={styles.overlay} onPress={() => setMenuOpen(false)} />
          <View style={[styles.popover, { top: insets.top + headerH + 6, right: 16 }]}>
            <TouchableOpacity style={styles.popoverItem} onPress={() => { setMenuOpen(false); setShowDispute(true); }}>
              <ThemedText style={styles.popoverText}>Create a dispute</ThemedText>
            </TouchableOpacity>
          </View>
        </Modal>
        ...
        <View style={[styles.composer, { marginBottom: 10 + insets.bottom }]}>
          <TouchableOpacity>
            <Image source={require('../../../assets/Vector (21).png')} style={styles.iconImg} />
          </TouchableOpacity>
          <TextInput
            style={styles.input}
            placeholder="Type a message"
          ...
          <TouchableOpacity onPress={handleSend}>
            <Image source={require('../../../assets/Vector (22).png')} style={styles.iconImg} />
          </TouchableOpacity>
        </View>
      </KeyboardAvoidingView>
...
const styles = StyleSheet.create({
  ...
  composer: {
    ...
    borderRadius: 20, borderWidth: 0.5, borderColor: '#CDCDCD',
  },
