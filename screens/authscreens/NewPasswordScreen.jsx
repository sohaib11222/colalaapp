...
//   },
// });
// screens/Auth/NewPasswordScreen.jsx
import React, { useState } from 'react';
import {
  View,
  TextInput,
...
const NewPasswordScreen = () => {
  ...
  const email = route.params?.email || ''; // passed from VerifyCode
  ...
  const handleProceed = async () => {
    ...
    if (!email || !code) {
      Alert.alert('Missing info', 'Email or verification code is missing.');
      return;
    }
    ...
    try {
      ...
      Alert.alert('Error', err?.message || 'Unable to reset password. Please try again.');
    }
  };

  return (
    <SafeAreaView style={styles.container}>
      {/* lift content on keyboard open */}
      <KeyboardAvoidingView
        style={{ flex: 1 }}
